  ...
  continue: {
    ...
    buildUserPrompt: ({
      ...
      return [
        ...
        // 编辑器续写目标字数上调到 1500-2000
        formatSection('输出要求', '请基于以上上下文（特别是章节计划中的目标与事件），生成本章后续内容。总字数控制在 1500-2000 左右，每段 250-500 字，信息密度高、情节推进明确，保持画面感与节奏感，避免水字。这一段必须承接上一段最后一句的情绪/动作，不要重新起头。每 2-3 段安排一次“缓冲段”（走路、观察、对话、内心）。只输出正文内容。')
      ].join('\n')
    }
  },
  polish: {
